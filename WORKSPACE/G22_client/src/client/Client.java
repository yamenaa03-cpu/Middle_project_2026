...
/**
 * The Client class extends the OCSF AbstractClient framework. It is responsible
 * for: - Connecting to the server - Sending requests (ReservationRequest
 * objects) - Receiving responses from the server (ReservationResponse) -
...
public class Client extends AbstractClient {
	...
	/**
	 * This method automatically runs whenever the server sends a message. It
	 * handles responses from the server as an instance of ReservationResponse
	 * object
	 * 
	 *
	...
	protected void handleMessageFromServer(Object msg) {
		if (msg instanceof UserAccountResponse) {
			...
			ui.handleUserAccountResponse(authResp);
			return;
		}

		if (msg instanceof ReservationResponse resResp) {
			ui.handleReservationResponse(resResp);
			if (!(resResp.getOperation() == ReservationOperation.GET_ACTIVE_RESERVATIONS
					|| resResp.getOperation() == ReservationOperation.GET_WAITLIST))
	...
	/**
	 * Sends a request to retrieve all active reservations from the database.
	 */
	public void requestAllReservations() {
		ReservationRequest req = ReservationRequest.createGetActiveReservationsRequest();
		sendRequest(req);
	}
	...
	public void requestUpdateReservation(int reservationId, LocalDateTime newDateTime, int newGuests) {
		ReservationRequest req = ReservationRequest.createUpdateReservationRequest(reservationId, newDateTime,
				newGuests);
		sendRequest(req);
	}
	...
	public void requestCreateGuestReservation(LocalDateTime reservationDateTime, int guestCount, String name,
		...
		ReservationRequest req = ReservationRequest.createCreateGuestReservationRequest(reservationDateTime, guestCount,
				name, phone, email);
		sendRequest(req);
	...
	/**
	 * Sends a request (to the sever) to Get the Cancellable reservations for a
	 * guest accourding to confirmation code.
	 *
	...
	// *********************WaitingList*********************
	public void requestWaitingListForGuest(int guests, String fullName, String phone, String email) {
	...
	/**
	 * sends a request object to the server. catches any Exception while doing so
	 *
